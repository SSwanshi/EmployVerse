...
const ApplyJob = () => {
  ...
  const handleResumeFileChange = async (e) => {
    ...
    try {
      const data = await applicantApi.uploadJobApplicationResume(jobId, file);
      if (data?.resumeId) {
  ...
  const handleApply = async () => {
    ...
    if (resumeOption === 'specific' && !applicationResumeId) {
      setError('Please upload a job-specific resume first.');
      return;
    }
    ...
    try {
      ...
      const response = await applicantApi.applyForJob(jobId, resumeId);
                    ...
                    <div className="space-y-3">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 text-blue-600"
                        />
                        <span>Use a job-specific resume</span>
                      </label>
